...
contract LiquidStakingManager is ILiquidStakingManager, Initializable, ReentrancyGuard, StakehouseAPI, ETHTransferHelper {
    ...
    function recoverSigningKey(
        ...
        address smartWallet = _onlyNodeRunnerOrDAO(_nodeRunner);
    ...
    function rageQuit(
        ...
        address smartWallet = _onlyNodeRunnerOrDAO(_nodeRunner);
        if (bytes4(_data) != IBalanceReporter.rageQuitKnot.selector && bytes4(_data) != IBalanceReporter.multipartyRageQuit.selector) revert InvalidSelector();
    ...
    function toggleHouseGatekeeper(bool _enabled) external onlyDAO {
        IStakeHouseRegistry(stakehouse).setGateKeeper(_enabled ? address(gatekeeper) : address(0));
    }
    ...
    /// @notice Liquid staking DAO can set the building type so that loot can be claimed by community
    ...
    /// @dev Reusable logic for gatekeeping certain methods to be only called by DAO or node runner
    function _onlyNodeRunnerOrDAO(address _nodeRunner) internal returns (address) {
