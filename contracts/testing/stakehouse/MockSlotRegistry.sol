...
contract MockSlotRegistry {
    ...
    mapping(bytes => uint256) slashedAmountOfSLOTForKnot;
    function setSlashedAmountOfSLOTForKnot(bytes calldata _blsPubKey, uint256 _slashedAmount) external {
        slashedAmountOfSLOTForKnot[_blsPubKey] = _slashedAmount;
    }

    function currentSlashedAmountOfSLOTForKnot(bytes calldata _blsPubKey) external view returns (uint256) {
        return slashedAmountOfSLOTForKnot[_blsPubKey];
    }
}