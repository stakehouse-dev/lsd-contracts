...
contract Syndicate is ISyndicateInit, Initializable, Ownable, ReentrancyGuard, StakehouseAPI, ETHTransferHelper {
    ...
    /// @notice Number of BLS public keys that can be activated whenever activateProposers() is called
    uint256 public constant ACTIVATION_BATCH_SIZE = 45;
    ...
    function activateProposers() public {
        ...
        if (numberOfActiveKnots > 0 && totalClaimed > 0) {
        ...
        numToActivate = numToActivate > ACTIVATION_BATCH_SIZE ? ACTIVATION_BATCH_SIZE : numToActivate;
        ...
        if (currentActivated == 0 || totalClaimed == 0) {
            updateAccruedETHPerShares();
        }
    }
    ...
    function unstake(
        ...
        for (uint256 i; i < numOfKeys; ++i) {
            ...
            if (!_hasBeenActivated(_blsPubKey)) revert InactiveKnot();
    ...
    function claimAsCollateralizedSLOTOwner(
        ...
        for (uint256 i; i < numOfKeys; ++i) {
            bytes calldata _blsPubKey = _blsPubKeys[i];
            if (!isKnotRegistered[_blsPubKey]) revert KnotIsNotRegisteredWithSyndicate();
            if (!_hasBeenActivated(_blsPubKey)) revert InactiveKnot();

            // process newly accrued ETH and distribute it to collateralized SLOT owners for the given knot
            _updateCollateralizedSlotOwnersLiabilitySnapshot(_blsPubKey, false);
    ...
    function batchUpdateCollateralizedSlotOwnersAccruedETH(bytes[] calldata _blsPubKeys) external {
        ...
        activateProposers();
        for (uint256 i; i < numOfKeys; ++i) {
            if (!_hasBeenActivated(_blsPubKeys[i])) revert InactiveKnot();
            _updateCollateralizedSlotOwnersLiabilitySnapshot(_blsPubKeys[i], false);
        }
    }
    ...
    function calculateUnclaimedFreeFloatingETHShare(bytes calldata _blsPubKey, address _user) public view returns (uint256) {
    ...
    function previewUnclaimedETHAsFreeFloatingStaker(
        ...
        // This will ensure that rewards accrued before activation are not considered
        uint256 adjustedClaimForActivation;
        if (!isNoLongerPartOfSyndicate[_blsPubKey] && sETHUserClaimForKnot[_blsPubKey][_staker] == 0) {
            adjustedClaimForActivation = (lastAccumulatedETHPerFreeFloatingShare[_blsPubKey] * stakedBal) / PRECISION;
        }

        return userShare - sETHUserClaimForKnot[_blsPubKey][_staker] - adjustedClaimForActivation;
    }
    ...
    function previewUnclaimedETHAsCollateralizedSlotOwner(
        ...
        uint256 previewToActivate;
        if (numberOfActiveKnots == 0 || totalClaimed == 0) {
            previewToActivate = previewActivateableProposers();
        }

        if (numberOfActiveKnots + previewToActivate == 0) return 0;
        ...
        uint256 accumulatedSoFar = accumulatedETHPerCollateralizedSlotPerKnot
                    + ((calculateETHForFreeFloatingOrCollateralizedHolders() - lastSeenETHPerCollateralizedSlotPerKnot) / (numberOfActiveKnots + previewToActivate));
        ...
        // Fetch information about the knot i.e. associated house
        ...
        currentAccrued += balance * unprocessedForKnot / 4 ether;
    ...
    function _updateCollateralizedSlotOwnersLiabilitySnapshot(bytes memory _blsPubKey, bool _forceUpdate) internal {
        // When the BLS public key is no longer part of the syndicate, return early since no more rewards can accrue to collateralised SLOT owners
        if (isNoLongerPartOfSyndicate[_blsPubKey] && !_forceUpdate) return;
        ...
        if (unprocessedETHForCurrentKnot > 0) {
            // Don't allocate ETH to collateralised owners when the current slashed amount is four
            if (currentSlashedAmount < 4 ether) {
                for (uint256 i; i < numberOfCollateralisedSlotOwnersForKnot; ++i) {
                    ...
                    // Maximum of 4 collateralized SLOT and hence the division by 4 ether
                    accruedEarningPerCollateralizedSlotOwnerOfKnot[_blsPubKey][collateralizedOwnerAtIndex] +=
                        (balance * unprocessedETHForCurrentKnot) / 4 ether;
                }
            }

            // when there is SLOT that is slashed but not topped up by anyone, allocate ETH to the contract bucket to later give
            // to the address that tops up the slot (which could also be the original SLOT owner)
            if (currentSlashedAmount > 0) {
                accruedEarningPerCollateralizedSlotOwnerOfKnot[_blsPubKey][address(this)] +=
                (currentSlashedAmount * unprocessedETHForCurrentKnot) / 4 ether;
            }
        ...
        // When the contract bucket for a knot was accruing ETH whilst SLOT that was slashed and not topped up,
        // then when the slot is fully topped up and ETH is claimed from the syndicate, allocate it all to the
        // most recent collateralised slot owner from a KNOT that topped up the knot.
        uint256 accruedEarningsContractBucket = accruedEarningPerCollateralizedSlotOwnerOfKnot[_blsPubKey][address(this)];
        if (currentSlashedAmount == 0 && accruedEarningsContractBucket > 0) {
            address collateralizedOwnerAtIndex = getSlotRegistry().getCollateralisedOwnerAtIndex(_blsPubKey, numberOfCollateralisedSlotOwnersForKnot - 1);
            accruedEarningPerCollateralizedSlotOwnerOfKnot[_blsPubKey][collateralizedOwnerAtIndex] += accruedEarningsContractBucket;
            accruedEarningPerCollateralizedSlotOwnerOfKnot[_blsPubKey][address(this)] = 0;
        }
    ...
    function _calculateNewAccumulatedETHPerFreeFloatingShare(uint256 _ethSinceLastUpdate, bool _previewFreeFloatingSharesToActivate) internal view returns (uint256) {
        uint256 sharesToActivate;

        if ((numberOfActiveKnots == 0 || totalClaimed == 0) && _previewFreeFloatingSharesToActivate) {
            sharesToActivate = previewTotalFreeFloatingSharesToActivate();
        }
    ...
    function _deRegisterKnots(bytes[] calldata _blsPublicKeys) internal {
        ...
        if (numOfKeys == 0) revert EmptyArray();
        for (uint256 i; i < numOfKeys; ++i) {
            bytes calldata blsPublicKey = _blsPublicKeys[i];

            // Prevent de-registration when knot is slashed and there is an amount of ETH in contract bucket that would be lost
            bool isDeRegistrationPermitted =
                accruedEarningPerCollateralizedSlotOwnerOfKnot[blsPublicKey][address(this)] > 0 ?
                getSlotRegistry().currentSlashedAmountOfSLOTForKnot(blsPublicKey) == 0 : true;

            require(isDeRegistrationPermitted, "Top up slot");
    ...
    function _deRegisterKnot(bytes memory _blsPublicKey) internal {
        ...
        if (!_hasBeenActivated(_blsPublicKey)) revert InactiveKnot();
        ...
        _updateCollateralizedSlotOwnersLiabilitySnapshot(_blsPublicKey, true);
    ...
    function _hasBeenActivated(bytes memory _blsPubKey) internal view returns (bool) {
        for (uint256 i; i < activationPointer; ++i) {
            if (keccak256(proposersToActivate[i]) == keccak256(_blsPubKey)) return true;
        }
        return false;
    }
}