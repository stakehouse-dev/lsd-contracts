...
import { ITransferHookProcessor } from "../../../contracts/liquid-staking/../interfaces/ITransferHookProcessor.sol";
import { ETHTransferHelper } from "../../../contracts/liquid-staking/../transfer/ETHTransferHelper.sol";
...
abstract contract GiantPoolBase is ITransferHookProcessor, ReentrancyGuardUpgradeable, ETHTransferHelper {
    ...
    event WithdrawalBatchAssociatedWithBLSKey(bytes32 key, uint256 indexed batchId);

    /// @notice Emitted when a withdrawal batch is disassociated with a BLS pub key
    event WithdrawalBatchDisassociatedWithBLSKey(bytes32 key, uint256 indexed batchId);
    ...
    mapping(bytes32 => bool) internal isBLSPubKeyFundedByGiantPool;

    /// @notice For a given BLS key, allocated withdrawal batch
    mapping(bytes32 => uint256) public allocatedWithdrawalBatchForBlsPubKey;

    /// @notice For a given withdrawal batch, allocated BLS key
    mapping(uint256 => bytes32) public allocatedBlsPubKeyForWithdrawalBatch;
    ...
    function withdrawETH(
        ...
        for (uint256 i = totalNumOfBatches; i > 0; --i) {
            ...
            if (allocatedBlsPubKeyForWithdrawalBatch[batchAtIndex] != 0) {
                continue;
            }
    ...
    function onMintDerivatives(bytes32 _blsPublicKey) external {
    ...
    function jumpTheQueue(uint256 _targetPosition, uint256 _existingPosition, address _user) external {
        ...
        if (allocatedBlsPubKeyForWithdrawalBatch[_targetPosition] != 0) revert BatchAllocated();
        if (allocatedBlsPubKeyForWithdrawalBatch[_existingPosition] != 0) revert BatchAllocated();
    ...
    function _onStake(bytes32 _blsPublicKey) internal virtual {
    ...
    function _allocateStakingCountToBlsKey(bytes32 _blsPublicKey, uint256 _count) internal {
    ...
    function _onBringBackETHToGiantPool(bytes32 _blsPublicKey) internal virtual {
    ...
    /// @notice Allow liquid staking managers to notify the giant pool about derivatives minted for a key
    function _onMintDerivatives(bytes32 _blsPublicKey) internal virtual {}
